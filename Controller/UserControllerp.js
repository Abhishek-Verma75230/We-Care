import Userp from "../model/Userp.js"

export const signupp=async (request,response)=>{
    try {
        ...
        const existingUser= await Userp.findOne({username});
       ...
       //   const hashPassword=await bcrypt.hash(password,10);
        //    password=hashPassword;
           ...
           const newUser=new Userp({name,username,password});
           await newUser.save();
...
export const loginuserp=async(request,response)=>{
    let user=await Userp.findOne({username:request.body.username});
       if(!user)
       {
